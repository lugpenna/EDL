...
} --Trabalho 6: Registro, guarda a dimensão e posicao da barra
...
bombas = {}
-- Trabalho 6: Table sendo usado como um Array, usado nas funcoes criaBomba e removeBombas
...
function love.update(dt)
	...
	if state == "playing" then
		...
		local direcao = 0
		if love.keyboard.isDown("d") then --Trabalho 6: Enum, a função love.keyboard.isDown 
			direcao = 1
		end
...
function love.draw()
	...
	elseif state == "playing" then 
		...
		if score < 5 then
			love.graphics.setColor(255, 0, 0, 255)
			love.graphics.printf("BABY LEVEL", 0, 450, 640, "center") --Trabalho 6: Tupla, conjunto finito e imutavel
		elseif score < 10 then
			love.graphics.setColor(255, 0, 0, 255)
